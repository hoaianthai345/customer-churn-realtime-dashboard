...
def main() -> None:
    ...
    rows = read_csv(settings.user_logs_clean_path)
    batches = group_rows_by_date(rows, "date")
    ...
    for idx, (event_date, day_rows) in enumerate(batches, start=1):
        ...
        producer.flush()
        logger.info("Replayed user log batch %s day=%s rows=%s", idx, event_date, len(day_rows))
        time.sleep(settings.replay_sleep_seconds)

    producer.close()
    logger.info("User log replay done. total_events=%s", total_events)
